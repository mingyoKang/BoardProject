...
public class BoardDeleteReqController implements Controller{
	...
	public void execute(HttpServletRequest req, HttpServletResponse resp) {
		// delete를 진행하기 전 password를 받아 DB에 저장된 password와 비교할 컨트롤러
		...
		if(flag !=null) { // flag에 값이 있을 때 // 더 정확하게는 init일 때는 처음 시행할 때
			...
			HttpUtil.forward(req, resp, "/WEB-INF/View/board/isDeleted.jsp");
			...
			if(conPassword.equals(vo.getPassword())) { // 비밀번호 일치
				...
				String url = "/Board/delete.do?number=" + number + "&nowPage=" + nowPage + "&start=" + start + "&end=" + end;
